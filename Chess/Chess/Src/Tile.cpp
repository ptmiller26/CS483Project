...
{
	if (tile.pPieceOnThisTile != NULL)
	{
		*pPieceOnThisTile = *tile.pPieceOnThisTile; 
	}
}
...
void Tile::ClonePieceOnThisTile(Piece *pieceToClone)
{
	if (pPieceOnThisTile != NULL)
	{
		Piece* newPiece;
		EWhichTeam eTempTeam = pieceToClone->eTeamColor;

		if (pieceToClone->ePieceType == ePawnType)
		{
			newPiece = new Pawn(eTempTeam);
		}
		else if (pPieceOnThisTile->ePieceType == eRookType)
		{
			newPiece = new Rook(eTempTeam);
		}
		else if (pPieceOnThisTile->ePieceType == eKnightType)
		{
			newPiece = new Knight(eTempTeam);
		}
		else if (pPieceOnThisTile->ePieceType == eBishopType)
		{
			newPiece = new Bishop(eTempTeam);
		}
		else if (pPieceOnThisTile->ePieceType == eQueenType)
		{
			newPiece = new Queen(eTempTeam);
		}
		else if (pPieceOnThisTile->ePieceType == eKingType)
		{
			newPiece = new King(eTempTeam);
		}

		this->pPieceOnThisTile = newPiece;
	}
}