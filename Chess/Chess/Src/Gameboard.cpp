...
Gameboard::~Gameboard()
{
	//delete pGameboardArrayOfTiles;
	//pGameboardArrayOfTiles = NULL;
}

Gameboard::Gameboard(const Gameboard& gameboard)
{
	// Allocating the memory first
	pGameboardArrayOfTiles = new Tile[64];

	// Copying the value from the passed object
	pGameboardArrayOfTiles = gameboard.pGameboardArrayOfTiles;
	memcpy(pGameboardArrayOfTiles, gameboard.pGameboardArrayOfTiles, sizeof(Tile) * 64);

	for (int i = 0; i < 64; i++)
	{
		if (pGameboardArrayOfTiles[i].pPieceOnThisTile != NULL)
		{
			pGameboardArrayOfTiles[i].ClonePieceOnThisTile(pGameboardArrayOfTiles[i].pPieceOnThisTile);
		}
	}
}

//Gameboard::Gameboard(Gameboard* pGameboard)
...
void Gameboard::SetPiecesUpOnTheBoard(HumanPlayer* player1, AIPlayer* AI)
{
